...
construct_index("D:/working/openAI/custom/data")
...
def history():
    ...
    cursor.execute("SELECT user_chat, ai_answer FROM convo WHERE userid = %s AND domain= %s", (uid,domain))
    messages = cursor.fetchall()
    
    return jsonify({'messages': messages,'intro':"The following is a conversation with an AI assistant. The assistant is helpful, creative, clever, and very friendly."})
...
def ask():
    ...
    if question:
        cursor = mysql.connection.cursor()
        cursor.execute("SELECT * FROM convo WHERE userid = %s AND domain= %s", (uid,domain))
        messages = cursor.fetchall()
        ...
        cursor.execute("INSERT INTO convo (datetime_created, domain, userid, user_chat, ai_answer) VALUES (%s, %s, %s, %s, %s)", (datetime_created, domain, uid, raw_question, response.response))
